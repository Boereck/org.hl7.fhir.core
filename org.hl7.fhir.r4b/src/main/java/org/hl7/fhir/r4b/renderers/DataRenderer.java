...
import org.hl7.fhir.r4b.model.DataRequirement.DataRequirementCodeFilterComponent;
import org.hl7.fhir.r4b.model.DataRequirement.DataRequirementDateFilterComponent;
import org.hl7.fhir.r4b.model.DataRequirement.DataRequirementSortComponent;
import org.hl7.fhir.r4b.model.DataRequirement.SortDirection;
...
import org.hl7.fhir.r4b.model.Timing.EventTiming;
import org.hl7.fhir.r4b.model.Timing.TimingRepeatComponent;
import org.hl7.fhir.r4b.model.Timing.UnitsOfTime;
...
public class DataRenderer extends Renderer {
  ...
  private String lookupCode(String system, String version, String code) {
    ValidationResult t = getContext().getWorker().validateCode(
        getContext().getTerminologyServiceOptions().withVersionFlexible(true), system, version, code, null);
  ...
  private boolean isCanonical(String path) {
    ...
    if (Utilities.existsInList(t, "ActivityDefinition", "CapabilityStatement", "ChargeItemDefinition", "Citation",
        "CodeSystem", "CompartmentDefinition", "ConceptMap", "ConditionDefinition", "EventDefinition", "Evidence",
        "EvidenceReport", "EvidenceVariable", "ExampleScenario", "GraphDefinition", "ImplementationGuide", "Library",
        "Measure", "MessageDefinition", "NamingSystem", "PlanDefinition"))
      return true;
  ...
  public void renderExtensionsInList(XhtmlNode ul, DataType element)
  ...
  public void renderExtensionsInList(XhtmlNode ul, BackboneType element)
      throws FHIRFormatError, DefinitionException, IOException {
  ...
  public void renderExtensionsInText(XhtmlNode div, DataType element, String sep)
      throws FHIRFormatError, DefinitionException, IOException {
    boolean first = true;
    for (Extension ext : element.getExtension()) {
      if (canRender(ext)) {
  ...
  public void renderExtensionsInList(XhtmlNode div, BackboneType element, String sep)
      throws FHIRFormatError, DefinitionException, IOException {
    boolean first = true;
    for (Extension ext : element.getModifierExtension()) {
      if (first) {
  ...
  public static String display(IWorkerContext context, DataType type) {
    return new DataRenderer(
        new RenderingContext(context, null, null, "http://hl7.org/fhir/R4", "", null, ResourceRendererMode.END_USER))
            .display(type);
  }
  ...
  private String displayDateTime(BaseDateTimeType type) {
    ...
    // locale - otherwise, format defaults to SHORT for the Locale (which defaults
    // to default Locale)
  ...
  private DateTimeFormatter getDateFormatForPrecision(BaseDateTimeType type) {
    ...
    if (context.isTechnicalMode()) {
      ...
      case MONTH:
        return new DateTimeFormatterBuilder().appendValue(YEAR, 4, 10, SignStyle.EXCEEDS_PAD).appendLiteral('-')
  ...
  private boolean isOnlyDate(TemporalPrecisionEnum temporalPrecisionEnum) {
    return temporalPrecisionEnum == TemporalPrecisionEnum.YEAR || temporalPrecisionEnum == TemporalPrecisionEnum.MONTH
  ...
  protected void renderCoding(XhtmlNode x, Coding c, boolean showCodeDetails) {
    ...
    if (showCodeDetails) {
      x.addText(s + " (Details: " + TerminologyRenderer.describeSystem(c.getSystem()) + " code " + c.getCode() + " = '"
          + lookupCode(c.getSystem(), c.getVersion(), c.getCode()) + "', stated as '" + c.getDisplay() + "')");
    } else
  ...
  private String displayIdentifier(Identifier ii) {
    ...
    if (ii.hasType()) {
      if (ii.getType().hasText())
        ...
        s = lookupCode(ii.getType().getCoding().get(0).getSystem(), ii.getType().getCoding().get(0).getVersion(),
            ii.getType().getCoding().get(0).getCode()) + ": " + s;
    } else {
      s = "id: " + s;
    }
  ...
  protected void renderQuantity(XhtmlNode x, Quantity q, boolean showCodeDetails) {
    ...
    if (q.hasUnit())
      ...
      // if there's a code there *shall* be a system, so if we've got one and not the
      // other, things are invalid and we won't bother trying to render
    ...
    if (showCodeDetails && q.hasCode()) {
      x.span("background: LightGoldenRodYellow", null)
          .tx(" (Details: " + TerminologyRenderer.describeSystem(q.getSystem()) + " code " + q.getCode() + " = '"
              + lookupCode(q.getSystem(), null, q.getCode()) + "')");
    }
  }
  ...
  public String displayRange(Range q) {
    ...
    boolean sameUnits = (q.getLow().hasUnit() && q.getHigh().hasUnit()
        && q.getLow().getUnit().equals(q.getHigh().getUnit()))
